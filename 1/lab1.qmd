...
```{r}
data <- fread(file = "./1/Kaggle_Social_Network_Ads.csv")
...
> What does the difference in performance between the standard logistic regression and the GAMs
suggest about the former? Is it over- or underfitted? Does it have high(er) bias or high(er) variance
compared to the GAMs?
> Which of the three GAMs do you prefer? Motivate.

```{r}
final_model <- lm(Purchased ~ ns(Age,4) + ns(Salary,4) + Gender, 
                  data = data)
