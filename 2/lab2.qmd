...
When investigating the variables closer no variable is exactly weighted with 0, but most of the variables are weighted with less than 10%. The random forest is averaging out the noise by taking the mean of all trees. The noise variables still have a little bit of weight, but are less influential in mass. 
...
randomForest::partialPlot(model_forest$finalModel, 
