...
In the first part of this lab, we will again consider the ad _purchase_ data that we used in lab 1 (containing information about individuals’ purchasing behavior under exposure to online ads). As in lab 1, we will build classification models to predict ad purchase (0/1) based on a set of covariates about individuals (including `Age`, `Gender` and `Salary`). In contrast to lab 1, we here focus on _non-parametric_ methods.
